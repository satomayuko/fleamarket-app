        ...
        <form class="sell__form" method="POST" action="{{ route('items.store') }}" enctype="multipart/form-data">
            ...
            <section class="block">
                ...
                <div class="subblock">
                    ...
                    <div class="chips">
                        @php
                            $old = collect(old('categories', []))->map(fn($v) => (int)$v);
                        @endphp

                        @foreach ($categories as $parent)
                            <input
                                type="checkbox"
                                id="cat_parent_{{ $parent->id }}"
                                name="categories[]"
                                value="{{ $parent->id }}"
                                class="chip-input"
                                {{ $old->contains($parent->id) ? 'checked' : '' }}
                            >
                            <label for="cat_parent_{{ $parent->id }}" class="chip">
                                {{ $parent->name }}
                            </label>
                            ...
                            @foreach ($parent->children as $child)
                                <input
                                    ...
                                    value="{{ $child->id }}"
                                    class="chip-input"
                                    {{ $old->contains($child->id) ? 'checked' : '' }}
                                >
                                <label for="cat_{{ $child->id }}" class="chip">
                                    {{ $child->name }}
                                </label>
                            @endforeach
                        @endforeach
