...
@section('content')
    ...
    <div class="profile-page">
        ...
        <div class="user-info">
            ...
            <a href="{{ route('mypage.profile.edit') }}" class="user-info__edit">
                プロフィールを編集
            </a>
        </div>
        ...
        <div class="products-grid">
            @forelse ($items as $item)
                <a href="{{ route('items.show', $item->id) }}" class="product-card">
                    <div class="product-image">
                        ...
                        @if ($src)
                            <img 
                                src="{{ $isUrl ? $src : asset('storage/' . $src) }}"
                                alt="{{ e($item->title) }}"
            ...
            @empty
                <div class="empty">
                    <p>
                        {{ $tab === 'sell' ? 'まだ出品はありません。' : 'まだ購入履歴はありません。' }}
                    </p>
                </div>
            @endforelse
        </div>
